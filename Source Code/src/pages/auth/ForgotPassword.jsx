...
export default function ForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(typeof err.response?.data?.message === 'string' ? err.response?.data?.message : err.response?.data?.message?.toString() || 'Failed to initiate recovery protocol');
        } finally {
            setLoading(false);
        }
    };
            ...
            >
                ...
                <div className="bg-white/80 dark:bg-dark-900/80 backdrop-blur-2xl p-8 md:p-12 rounded-[3rem] shadow-3xl border border-white/20 dark:border-dark-800 relative group overflow-hidden">
                    ...
                    <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic mb-8 flex items-center gap-3">
                        <KeyIcon className="w-6 h-6 text-indigo-600" /> Vector Recovery
                    </h2>
                    ...
                    {success ? (
                        <div className="text-center space-y-6">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed italic">
                                A secure neural link has been transmitted to your primary identifier. Please verify your inbox to initialize the cipher reset.
                            </p>
                            <Link
                                to="/login"
                                className="inline-flex items-center gap-2 text-[10px] font-black text-indigo-600 hover:text-indigo-700 uppercase tracking-[0.2em] italic border-b-2 border-transparent hover:border-indigo-600 transition-all pt-4"
                            >
                                Return to Uplink Interface
                            </Link>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center italic px-4 leading-relaxed">
                                Provide your registered domain address to receive a structural recovery vector for your access cipher.
                            </p>
                    ...
                    <div className="mt-10 pt-8 border-t border-slate-100 dark:border-dark-800 text-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                            Recollected your Access Cipher?
                            <Link to="/login" className="ml-2 text-indigo-600 hover:text-indigo-700 underline decoration-dotted underline-offset-4">Authenticate Primary Node →</Link>
                        </p>
                    </div>
                </div>
                ...
                <div className="mt-12 flex justify-center gap-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Recovery Node: Ready</span>
                    </div>
                </div>
            </motion.div>
        </div>
